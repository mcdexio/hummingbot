...
from hummingbot.connector.derivative.mcdex.mcdex_derivative import McdexDerivative
...
class McdexDerivativeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.connector: McdexDerivative = McdexDerivative(
            [trading_pair],
            "PRIVATE_KEY_HERE",
            "",
            {trading_pair: "00001"})
        print("Initializing McdexDerivative market... this will take about a minute.")
    ...
    def test_update_balances(self):
        ...
        self.assertTrue(all_bals["USDC"] > 0)
    ...
    async def _test_get_quote_price(self):
        mcdex = self.connector
        buy_price = await mcdex.get_quote_price(trading_pair, True, Decimal("1"))
        self.assertTrue(buy_price > 0)
        print(f"buy_price: {buy_price}")
        sell_price = await mcdex.get_quote_price(trading_pair, False, Decimal("1"))
    ...
    def test_open_and_close_long_position(self):
        ...
        amount = Decimal("0.1")
        price = Decimal("10")
        order_id = mcdex.buy(trading_pair, amount, OrderType.LIMIT, price, position_action=PositionAction.OPEN)
        ...
        order_id = mcdex.sell(trading_pair, amount, OrderType.LIMIT, price, position_action=PositionAction.CLOSE)
    ...
    def test_open_position_failure(self):
        mcdex = self.connector
        # Since we don't have 1000000 USDC, this should trigger order failure
        amount = Decimal("1000000")
        price = Decimal("10")
        order_id = mcdex.sell(trading_pair, amount, OrderType.LIMIT, price, position_action=PositionAction.OPEN)
